...
def main():
    ...
    if run_eda:
        eda_pipeline = eda(project_root=project_root, plot_dir=result_plot_dir)
        eda_pipeline.run()

    if run_logreg:
        log_reg_pipeline = logregressionpipeline(project_root=project_root, plot_dir=result_plot_dir)
        log_reg_pipeline.run()

    if run_paper_plots:
        paper_pipeline = recreateplotsfromstrack(project_root=project_root,plot_dir=result_plot_dir)
        paper_pipeline.run()
